...
class _AdminTicketsScreenState extends State<AdminTicketsScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AdminBloc, AdminState>(
      buildWhen: (prev, curr) =>
          curr is TicketsLoaded || curr is AdminLoading || curr is AdminError,
                  ...
                  Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () =>
                            widget.bloc.add(LoadTickets(status: filterStatus)),
                      ),
                    ],
                  ),
                ],
                    ...
                    .map(
                      (t) => DataRow(
                        color: WidgetStateProperty.resolveWith((states) {
                          if (t.isOverdue)
                            return ThemeColors.unifiedDanger.withValues(
                              alpha: 0.04,
                            );
                          return null;
                        }),
                                ...
                                [
                                  if (t.lastAction != null)
                                    t.lastAction!
                                        .replaceAll('_', ' ')
                                        .toUpperCase(),
                                  if (t.lastActedByDeptName != null)
                                    t.lastActedByDeptName!,
                                ].join(' · '),
                                overflow: TextOverflow.ellipsis,
                              ...
                              children: [
                                ...
                                if (t.immediateChildCount > 0)
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: ThemeColors.unifiedAccent.withValues(
                                        alpha: 0.1,
                                      ),
                                      borderRadius: BorderRadius.circular(10),
                                    ),
                                    child: Text(
                                      '${t.immediateChildCount}',
                                      style: const TextStyle(
                                        fontSize: 10,
                                        fontWeight: FontWeight.w700,
      ...
      child: Row(
        ...
        children: [
          IconButton(
            ...
            onPressed: page > 1
                ? () => bloc.add(UpdateTicketPage(page - 1))
                : null,
          ),
          const SizedBox(width: 8),
          Text(
  ...
  void _openEdit(BuildContext context, AdminTicketModel ticket) async {
    ...
    showDialog(
      ...
      builder: (_) => BlocProvider.value(
        value: bloc,
        child: AdminTicketEditDialog(ticket: ticket, departments: departments),
      ),
    );
  }
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              dialogContext.read<AdminBloc>().add(
                DeleteTicket(t.id, dialogContext),
              );
            },
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: BlocBuilder<AdminBloc, AdminState>(
          buildWhen: (prev, curr) =>
              curr is TicketDetailLoaded || curr is AdminLoading,
          builder: (context, state) {
            if (state is AdminLoading) {
            ...
            return Column(
              ...
              children: [
                ...
                _infoRow(
                  'Assigned Dept',
                  Text(
                    '${t.assignedDeptName ?? '-'} (${t.assignedDeptCode ?? '-'})',
                    style: const TextStyle(fontSize: 13),
                  ),
                ),
                _infoRow(
                  'Assigned To',
                  Text(
                    t.assignedToName ?? ConstStrings.unassigned,
                    style: const TextStyle(fontSize: 13),
                  ),
                ),
                if (t.dueDate != null)
                ...
                if (t.version != null)
                  _infoRow(
                    'Version',
                    Text('${t.version}', style: const TextStyle(fontSize: 13)),
                  ),
                if (t.description != null && t.description!.isNotEmpty) ...[
                  const SizedBox(height: 8),
                  const Text(
                    ConstStrings.description,
                    style: TextStyle(
                      fontSize: 12,
                ...
                if (subTickets.isNotEmpty) ...[
                  const SizedBox(height: 16),
                  Text(
                    '${ConstStrings.subTicketCount} (${subTickets.length})',
                    style: const TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w700,
