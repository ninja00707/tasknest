...
UPDATE tickets SET ticket_number = CONCAT('UMP-TKQ-', LPAD(id::text, 3, '0'))
...
UPDATE tickets t SET ticket_number = CONCAT(
  ...
  '-SUB-',
  LPAD((SELECT COUNT(*) FROM tickets WHERE parent_ticket_id = t.parent_ticket_id AND id <= t.id)::text, 3, '0')
) WHERE ticket_number IS NULL AND parent_ticket_id IS NOT NULL;

SELECT SETVAL('ticket_number_seq', GREATEST(COALESCE((SELECT MAX(CAST(SPLIT_PART(ticket_number, '-', 3) AS INTEGER)) FROM tickets WHERE parent_ticket_id IS NULL AND ticket_number ~ '^UMP-TKQ-\d{3}$'), 0), 1));
