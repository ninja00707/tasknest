...
class TicketRepository {
  async getTicketDetails(ticketId) {
    const result = await pool.query(`
      SELECT
        ...
        t.parent_id, t.overall_progress,
  ...
  async getVisibleTickets(user, filters = {}) {
    ...
    if (user.role === 'ceo') {
      ...
      whereClause = `WHERE (
        ...
        OR t.parent_id IN (SELECT id FROM tickets WHERE assigned_dept_id = ${deptParam})
      )`;
    }
  ...
  async getDashboardStats(user) {
    ...
    if (user.role !== 'ceo') {
      ...
      whereClause = `WHERE (
        ...
        OR parent_id IN (SELECT id FROM tickets WHERE assigned_dept_id = $1)
      )`;
    }
  ...
  async getTicketById(ticketId, user) {
    ...
    if (!isAssignedDept && !isCreatorDept && !isTransferrer && !isResolver) {
        if(ticket.parent_id){
            const parentTicket = await this.getTicketDetails(ticket.parent_id);
            if(Number(parentTicket.assigned_dept_id) !== Number(user.department_id)){
  ...
  // ── Create ticket ─────────────────────────────────────────────────────────
  async createTicket({ title, description, priority, assignedDeptId, dueDate, createdBy, assignedToId, parentId }) {
    ...
    const result = await pool.query(`
      ...
      RETURNING id
    `, [title, description, priority, assignedDeptId, dueDate || null, createdBy.id, createdBy.department_id, assignedToId || null, status, parentId || null]);
