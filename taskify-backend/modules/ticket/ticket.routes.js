...
const { authenticate, isManager, isDirector, isEmployee } = require('./Auth.middleware');

// All ticket routes require authentication
router.use(authenticate);

// ── Analytics (MUST be defined above /:id) ───────────────────
router.get('/analytics/by-department', isDirector, controller.getAnalyticsByDepartment);
router.get('/analytics/organization', isDirector, controller.getOrganizationAnalytics);
router.get('/analytics/department/:departmentId', isManager, controller.getDepartmentAnalytics);

// ── Stats & Departments ───────────────────────────────────────
router.get('/stats', controller.getStats);
router.get('/departments', controller.getDepartments);
router.get('/employees', isEmployee, controller.getEmployees);
