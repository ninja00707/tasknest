...
(async () => {
  ...
  const r3 = await pool.query(`SELECT pid, query, state, wait_event, age(now(), query_start) AS running_for FROM pg_stat_activity WHERE state = 'active' AND query NOT LIKE '%pg_stat%' LIMIT 10`);
